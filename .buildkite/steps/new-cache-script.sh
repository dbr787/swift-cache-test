...
case "${1:-resolve}" in
  ...
  resolve)
    ...
    ;;
  ignore)
    list_cache  # List cache contents before ignoring cache and resolving dependencies
    resolve_dependencies_without_cache
    list_cache  # List cache contents after resolving dependencies (even though cache was ignored)
    ;;
  *)
